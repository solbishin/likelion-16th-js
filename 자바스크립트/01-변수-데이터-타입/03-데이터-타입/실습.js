...
// JavaScript안에서 안전하게 사용 가능한 큰/작은 함수
...
// 선언만 하고 값을 할당하지 않음
// 1. 선언
// 2. 초기화 : 의도적으로 값을 할당하지 않음(엔진이 초기값으로 undefied 할당)

let memoryStick = undefined
...
// 고유햔 값인가? (조건: proudctAIdString와 productBIdString가 값이 달라야 한다.)
// 비교 연산자 (두 값을 비교 -> 불리언 값 평가)
// 두 값이 같다, true 불리언 평가 (고유하지 않다.)
console.log('문자열을 사용할 때 고유하지 않은가?:', productAIdString == productBIdString) 
console.log('심볼을 사용할 때 고유하지 않은가?:', productAIdSymbol == productBIdSymbol) 
...
// 기능 사용 (실행, 호출))
console.log(커피_만들기('케냐', '뜨거운 물', false))
